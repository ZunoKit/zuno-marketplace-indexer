...
import type { ApiAbiDetail, ApiContract, ApiNetwork, Result } from "../../core/types";
import { sanitizeConfigKey } from "../../core/utils/helpers";
import { getZunoApiClient } from "../api/zunoApiClient.service";
...
export class ConfigBuilderService {
  ...
  /**
   * Parse deployed date to block number or use "latest"
   */
  private parseStartBlock(deployedAt: string): number | string {
    // For now, use "latest" to avoid block number lookup
    // In production, you'd query the block number from the deployment timestamp
    return 'latest';
  }
  ...
  async build(): Promise<Result<PonderConfig>> {
    ...
    const activeContracts = contracts.filter(c => activeNetworkIds.has(c.networkId) && c.isVerified);

    console.log(`[ConfigBuilder] ${activeContracts.length} active contracts after filtering`);
