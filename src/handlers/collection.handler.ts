...
import type { ERC721CollectionCreatedEvent, ERC1155CollectionCreatedEvent } from "../core/types/events";
...
export async function handleERC721CollectionCreated(event: any, context: any) {
  const args = event.args as ERC721CollectionCreatedEvent;
  const contractAddress = event.log.address;

  logger.logEventStart("ERC721CollectionCreated", contractAddress, event.block.number, event.transaction.hash);
  ...
  try {
    ...
    // Log the event
    await eventLogRepo.createFromEvent("ERC721CollectionCreated", contractAddress, null, args, {
      block: event.block,
    ...
    const collectionId = generateCollectionId(context.network.chainId, args.collectionAddress);
...
export async function handleERC1155CollectionCreated(event: any, context: any) {
  const args = event.args as ERC1155CollectionCreatedEvent;
  const contractAddress = event.log.address;

  logger.logEventStart("ERC1155CollectionCreated", contractAddress, event.block.number, event.transaction.hash);
  ...
  try {
    ...
    // Log the event
    await eventLogRepo.createFromEvent("ERC1155CollectionCreated", contractAddress, null, args, {
      block: event.block,
      transaction: event.transaction,
      log: event.log,
    });
